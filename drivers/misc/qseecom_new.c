			...
			const void *req_buf, void *resp_buf)
{
	...
	pr_debug("qseecom_scm_call2: svc_id = %d, tz_cmd_id = %d, cmd_id = %d\n",
		 svc_id, tz_cmd_id, cmd_id);
...
static int qseecom_load_app(struct qseecom_dev_handle *data, void __user *argp)
{
	...
	pr_debug("FINDBUGS: qseecom_load_app called\n");
	/* Copy the relevant information needed for loading the image */
	...
	if (app_id) {
		...
		first_time = true;
		pr_debug("loading apps for the first time debug message\n");
...
long qseecom_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
	...
	pr_debug("qseecom_ioctl: cmd = %d\n", cmd);

	switch (cmd) {
	case QSEECOM_IOCTL_REGISTER_LISTENER_REQ: {
                pr_debug("qseecom_ioctl: cmd = QSEECOM_IOCTL_REGISTER_LISTENER_REQ\n");
	...
	case QSEECOM_IOCTL_UNREGISTER_LISTENER_REQ: {
	        pr_debug("qseecom_ioctl: cmd = QSEECOM_IOCTL_UNREGISTER_LISTENER_REQ\n");
	...
	case QSEECOM_IOCTL_SEND_CMD_REQ: {
  	        pr_debug("qseecom_ioctl: cmd = QSEECOM_IOCTL_SEND_CMD_REQ\n");
		if ((data->client.app_id == 0) ||
			(data->type != QSEECOM_CLIENT_APP)) {
	...
	case QSEECOM_IOCTL_RECEIVE_REQ: {
	        pr_debug("qseecom_ioctl: cmd = QSEECOM_IOCTL_RECEIVE_REQ\n");
		if ((data->listener.id == 0) ||
			(data->type != QSEECOM_LISTENER_SERVICE)) {
	...
	case QSEECOM_IOCTL_SEND_RESP_REQ: {
	        pr_debug("qseecom_ioctl: cmd = QSEECOM_IOCTL_SEND_RESP_REQ\n");
		if ((data->listener.id == 0) ||
			(data->type != QSEECOM_LISTENER_SERVICE)) {
	...
	case QSEECOM_IOCTL_SET_MEM_PARAM_REQ: {
	        pr_debug("qseecom_ioctl: cmd = QSEECOM_IOCTL_SET_MEM_PARAM_REQ\n");
		if ((data->type != QSEECOM_CLIENT_APP) &&
	...
	case QSEECOM_IOCTL_LOAD_APP_REQ: {
	        pr_debug("qseecom_ioctl: cmd = QSEECOM_IOCTL_LOAD_APP_REQ\n");
		if ((data->type != QSEECOM_GENERIC) &&
			(data->type != QSEECOM_CLIENT_APP)) {
