		...
		case QSEOS_CLIENT_SEND_DATA_COMMAND_WHITELIST: {
			...
			if (qseecom.qsee_version < QSEE_VERSION_40) {
				...
				desc.args[6] = req_64bit->sglistinfo_len;
	                ...
	                pr_debug("Making actual scm call\n");
			ret = scm_call2(smc_id, &desc);
			pr_debug("Returned from actual call, ret = %d\n", ret);
			break;
		}
...
static int qseecom_init(void)
{
	...
	// Added code
	bool err = false;
	pr_warn("qseecom driver being initialized. Creating /sys/kernel/qseecom_debug\n");
...
#endif
	pr_warn("qseecom_init completed\n");
